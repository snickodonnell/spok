...
Status: **In progress 2026-07-13.** User-facing vocabulary presents Missions, Spok is explicitly the leader, and leader checkpoints, blockers, evidence gaps, budgets, and canonical state reasons precede the collapsed optional team map. The control room loads durable Mission v1 records through a dedicated client, including milestones, work items, authority/budget receipts, evidence, checkpoints, and recovery states. The new receipt/scheduler API now proves dependency, worktree, authority, repository-lock, provider-capacity, and protected-reserve decisions and distinguishes requested lanes from provider-emitted active lanes. Internal `enterprise` identifiers remain migration-only. The finding stays open for automatic dispatch/supervision wiring, presentation of the new decision state, full direct interaction/accessibility evidence, and removal of remaining legacy orchestration paths.
...
Status: **In progress 2026-07-13; bounded artifact progress slice 2026-07-17.** The provider foundation exposes a sanitized, fingerprinted CLI capability snapshot and fail-closed requirement result, including optional real leader health and explicit unknown auth state. The v1 run compiler pins that snapshot, rejects unsupported/ambiguous launch contracts, enforces exact unattended sessions and leaf isolation/delegation rules, and produces a redacted launch receipt. Foreground/background prompt launches now use content-verified managed artifacts instead of prompt argv; typed specialist reports become evidence-only events with a single bounded format repair. Durable work-item receipts and the readiness scheduler expose requested versus provider-emitted lanes, queue depth, repository locks, and protected execution/integration/recovery budgets at the API boundary. Bounded artifact runs now add shard/artifact counts and phase to transport heartbeats and exact-session status, persist resume findings, and refuse terminal success when the required handoff is missing or invalid. UX-023 remains open until those contracts drive automatic mission dispatch and outcome-first UI, and truthful attempt projection, supervision, synthesis, and previewed cleanup are wired and dogfooded.
