...
Status: **In progress 2026-07-13.** The provider foundation now exposes a sanitized, fingerprinted CLI capability snapshot and a fail-closed requirement result, including optional real leader health and explicit unknown auth state; raw inspect paths do not become mission UI state. The v1 run-spec compiler now pins that snapshot, rejects unsupported/ambiguous launch contracts, enforces exact unattended session and leaf isolation/delegation rules, and produces a redacted launch receipt. This removes remembered CLI flags and user-visible raw prompt/schema argv from the new launch path, but UX-023 remains open until prompt artifacts and current callers migrate to it and mission attachment, real-lane/reserve projection, supervision, synthesis, and previewed cleanup are wired and dogfooded.
