...
- A repository-wide agent operating contract plus canonical Grok CLI and agent-orchestration skills. The first runtime correction slice now adds a versioned, fingerprinted, sanitized Grok capability snapshot and compatibility gate with optional leader health, explicit unknown auth state, bounded provider output, trusted/audited route access, and supported/unsupported fixtures. Run-spec compilation, mission attachment, prompt artifacts, and scheduler work remain on the correction track.
...
4. Replace ad hoc Grok argv/prose with a versioned run spec: capability snapshot, prompt file/JSON, exact session intent, structured stream/report, leaf `--no-subagents`, bounded turns/tools/policy, redacted audit receipt, and native worktree/session cleanup. **In progress:** CLI-001 landed the bounded v1 capability snapshot, fingerprint, trusted runtime endpoint, argv-requirement inference, and fail-closed compatibility result. CLI-002 landed the immutable v1 run spec, deterministic argv compiler, capability-fingerprint revalidation at the trusted spawn boundary, exact session/leaf isolation rules, mutually exclusive stream/report output, and redacted approval/audit/process receipts. Durable mission/run attachment, runtime prompt artifact lifecycle/content verification, legacy caller migration, and native cleanup remain.
