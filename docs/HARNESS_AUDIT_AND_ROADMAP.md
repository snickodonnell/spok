...
- Silent long-running provider tools now retain their managed stream through transport-only heartbeats. Authenticated exact-session process status survives stream detachment for bounded recovery, duplicate active-session launch fails closed, and disconnect no longer implies stop; explicit scoped cancellation remains authoritative.
- A bounded artifact-workflow v1 slice now accepts an exact, hash-pinned `input-packet.v1.json` for one leader/report run. It records non-empty temporal shard completion and verified artifact receipts in the durable Mission directory, skips verified shards on exact-session resume, exposes checkpoint progress on transport heartbeats/status, and never creates a provider turn for observation. Its pre-terminal gate independently checks result identity, required files/hashes, JSON/JSONL shape, chronology, evidence linkage, Wallaby transcript-attribution invariants, and required-repair recurrence before allowing terminal success. This is a focused Archive-compatible execution contract, not yet the generic ORCH-003 packet builder or full synthesis engine.
...
- A thin versioned **Mission v1** domain and privileged API (`src/lib/missions`, `/api/missions`) with milestones, work items, dependency/evidence rules, authority and budget receipts, checkpoint materialization, a bounded receipt compiler, and a deterministic readiness scheduler. The user-facing Missions control room now loads durable Mission v1 evidence through the mission client; automatic dispatch consumption and supervision breadth remain open.
...
- A repository-wide agent operating contract plus canonical Grok CLI and agent-orchestration skills. CLI-001 through CLI-004 now provide capability/fingerprint gating, immutable run specs, deterministic managed prompt artifacts, exact sessions, bounded leaf policy, exclusive stream/report modes, compact typed specialist reports, sanitized launch evidence, and migrated foreground/background prompt callers. ORCH-001/ORCH-002 add durable delegation receipts and a dependency/capacity/isolation/lock/reserve decision API; compact context packets, automatic dispatch consumption, truthful attempt projection, supervision, synthesis, and cleanup remain on the correction track.
...
2. Add durable milestones and work items with dependencies, owner, requested capability, authority, time/token/tool budget, expected evidence, retries, and terminal outcome. **Domain/API and receipt/scheduler boundary landed** with validation that refuses false dependency completion without evidence, keeps terminal provenance distinct from process exit, compiles one bounded receipt per work item, and exposes deterministic readiness decisions. Automatic dispatch and UI wiring remain open.
...
4. Replace ad hoc Grok argv/prose with a versioned run spec: capability snapshot, prompt file/JSON, exact session intent, structured stream/report, leaf `--no-subagents`, bounded turns/tools/policy, redacted audit receipt, and native worktree/session cleanup. **In progress:** CLI-001/CLI-002 landed the bounded capability snapshot, fingerprint, immutable run spec, deterministic argv compiler, launch-boundary revalidation, exact sessions, leaf isolation/delegation rules, exclusive stream/report output, and redacted receipts. CLI-003/CLI-004 now add deterministic content-verified prompt artifacts with cleanup/recovery, migrate foreground/background prompt launches, validate compact specialist reports, emit evidence-only report events, and bound malformed-format repair to one context-free turn. Direct mission-run attachment and native session/worktree cleanup remain.
5. Add a scheduler for ready work items with dependency, provider capacity, repository-lock, verified worktree, and reserved integration/recovery budget constraints. Spok may parallelize only independent work. **Decision boundary landed:** ORCH-001 compiles and persists bounded mission/work-item receipts with exact execution identity, non-overlapping scope, parent authority/budget validation, and protected integration/recovery reserves. ORCH-002 deterministically selects only eligible work, distinguishes requested from provider-emitted active lanes, reports queue/reserve pressure, and avoids head-of-line starvation. Automatic dispatch and mission-control projection remain.
6. Add checkpoint summaries that make a mission resumable without replaying its full transcript: completed, active, blocked, changed assumptions, evidence, risks, budget use, cleanup state, and next decisions. **Focused slice landed:** bounded artifact workflows persist exact input identity, verified temporal shards, verified artifact receipts, validation findings, and handoff state under the durable Mission directory; generic mission checkpoint deltas remain open.
7. Add compact context packets and artifact references so agents receive the smallest sufficient context, not the whole project history; measure packet and return size. **Focused slice landed:** the bounded artifact workflow consumes one hash-pinned packet and explicit file references without recursive inventory or prompt embedding; generic relevance selection and byte/token telemetry remain open.
