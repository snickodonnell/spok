...
CLI-003 and CLI-004 landed on 2026-07-13. Foreground and unattended prompt launches now use a privileged prompt-bearing request that creates a deterministic, content-verified runtime artifact and passes only its managed path to the run compiler. Prompt bodies stay out of process receipts, audits, approvals, process state, and user-visible command events; ephemeral success/cancellation artifacts are removed, diagnostic failures are retained, and stale crash/approval artifacts are recoverable. The alternative report path compiles Grok's JSON-only schema mode, validates compact complete/partial/blocked/failed specialist reports, redacts fields, materializes evidence-only `agent_report` events, and permits one repository-context-free format repair for malformed output.

ORCH-001 and ORCH-002 landed on 2026-07-13 as the durable planning and readiness boundary. Mission intent compiles into versioned mission/work-item receipts with exact worktrees and sessions, owned/excluded scope, bounded authority, turn/token/retry limits, checks, return conditions, one integration owner, at least 20% integration reserve, and a separate recovery reserve. The privileged receipt and scheduling APIs revalidate workspace trust and choose ready work deterministically across dependency, verified isolation, approval, provider capacity, repository locks, and execution budget. Requested lanes remain distinct from provider-emitted active lanes, and an unaffordable high-priority item cannot starve smaller useful work. Automatic launch consumption and mission-control presentation remain subsequent orchestration/UX work.
...
2. Introduce a versioned `GrokRunSpec` and compiler. It owns cwd/worktree, prompt transport, session intent, model/effort, maximum turns, tool/web/sandbox/permission policy, subagent policy, output format, final report schema, and debug retention. **Landed 2026-07-13:** strict v1 parsing, immutable compiled state, deterministic capability-supported argv, exact new/resume/fork identity, attended-only continue-latest, stream/report exclusivity, leaf isolation/delegation denial, sanitized receipts, launch-boundary revalidation, and exact redacted approval fingerprints are covered by table-driven success and denial tests.
3. Replace large `-p` mission argv with runtime-managed `--prompt-file`/`--prompt-json` artifacts. Store only redacted metadata and a content hash in logs; delete ephemeral prompt files after durable handoff. **Landed 2026-07-13:** deterministic per-run artifact identity, strict containment/hash verification, mode-restricted atomic writes, idempotent retry, success/cancellation cleanup, diagnostic retention, stale recovery, and foreground/background caller migration are covered by long-prompt, tamper, redaction, cleanup, and crash fixtures.
4. Keep `streaming-json` for live ingestion and add a typed specialist return path. Because Grok's `--json-schema` implies JSON rather than streaming output, the run compiler must choose explicitly between a live stream and a JSON-only/report turn instead of emitting incompatible intent. Preserve unrecognized raw events without feeding them into every leader turn. **Landed 2026-07-13:** `specialist-v1` accepts bounded complete/partial/blocked/failed reports, normalizes them into evidence-only stream events, and issues one format-only repair packet without repository replay when malformed.
...
| CLI-003 | P0 | Runtime-managed prompt file/JSON lifecycle — **landed 2026-07-13** | session-start/runtime artifacts | long prompt, secret redaction, cleanup, crash recovery |
...
The next correction item is ORCH-003: compact context packets with byte/token telemetry and relevance/deduplication proof.
